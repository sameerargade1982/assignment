package com.argade.sameer.activemq;
...
public class ActiveMQMessageProducer {
	...
	private static final String INPUT_INTEGER = "input_int";
	...
	private TextMessage buildTextMessageWithProperty(final int inputInt) throws JMSException {
		Gson gson = new Gson();
		String eventMsg = "" + inputInt;
		TextMessage textMessage = session.createTextMessage(eventMsg);

		Random rand = new Random();
		int value = rand.nextInt(100);
		textMessage.setIntProperty(INPUT_INTEGER, inputInt);
		textMessage.setStringProperty(ACTION_ID_HEADER, String.valueOf(value));
	...
	public void sendMessage(int intValue) throws JMSException {
		TextMessage textMessage = buildTextMessageWithProperty(intValue);
		//setDdestination(false, newDestination );
		msgProducer.send(destination, textMessage);
	}
